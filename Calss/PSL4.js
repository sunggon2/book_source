// Person-Switch-Light-Fan-TV (캡슐화)
...
class Fan extends Device {
  ...
  set speed(value) {
    if (!this.isOn) return console.log('팬이 꺼져 있습니다.');
    if (!Number.isInteger(value) || value < 1 || value > this.#maxSpeed) {
      console.log(`⚠ 속도는 1 ~ ${this.#maxSpeed} 사이여야 합니다.`);
      return;
    }
    this.#speed = value;
    console.log(`속도가 ${this.#speed}로 설정되었습니다.`);
  }
...
class TV extends Device {
  ...
  onEffect() {
    console.log(`${this.name} 전원 ON (채널 ${this.#channel})`);
  }
  offEffect() {
  ...
  changeChannel(newChannel) {
    if (!this.isOn) {
      console.log(`${this.name}가 꺼져 있어 채널을 바꿀 수 없습니다.`);
      return;
    }
    this.#channel = newChannel;
    console.log(`${this.name} 채널 변경: ${this.#channel}`);
  }
}
